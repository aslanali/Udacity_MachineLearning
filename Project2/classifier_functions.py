...
def build_classifier(model, input_units, hidden_units, dropout):
    # Weights of pretrained model are frozen so we don't backprop
    for param in model.parameters():
        param.requires_grad = False

# defining the feed forward network as a classier with ReLU activations and dropout
    classifier = nn.Sequential(OrderedDict([
                             ('fc1',nn.Linear(25088,4096)),
                             ('relu1',nn.ReLU()),
                             ('dropout1',nn.Dropout(p=0.44)),
                             ('fc2',nn.Linear(4096,512)),
                             ('relu2',nn.ReLU()),
                             ('dropout2',nn.Dropout(p=0.35)),
    ...
    for e in range(epochs):
        ...
        for ii, (inputs, labels) in enumerate(trainloader):
            ...
            if steps % print_every == 0:
                ...
                Valid Accuracy: {round(float(accuracy/len(validloader) *100),3)}")
